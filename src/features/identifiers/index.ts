...
export const creators = {
    add: (): IdentifierAction => ({ type: ADD }),
    update: (i: number, identifiers: string): IdentifierAction => ({ type: UPDATE, i, identifiers }),
    remove: (i: number): IdentifierAction => ({ type: REMOVE, i }),
    select: (annotation: Annotation): IdentifierAction => ({ type: SELECT, annotation }),
}

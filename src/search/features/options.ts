...
export const creators = {
    setHH: (hh: boolean): OptionsAction => ({ type: HH, hh }),
    setVH: (vh: boolean): OptionsAction => ({ type: VH, vh }),
    setNeighbors: (neighbors: boolean): OptionsAction => ({ type: NEIGHBORS, neighbors }),
    setPublications: (threshold: number): OptionsAction => ({ type: PUBLICATIONS, threshold }),
    setMethods: (threshold: number): OptionsAction => ({ type: METHODS, threshold }),
}
